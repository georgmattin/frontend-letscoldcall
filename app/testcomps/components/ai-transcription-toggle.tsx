...
interface AITranscriptionToggleProps extends React.HTMLAttributes<HTMLDivElement> {
...
export default function AITranscriptionToggle({
  ...
  className,
  ...rest
}: AITranscriptionToggleProps) {
  const [expanded, setExpanded] = React.useState(defaultExpanded)
  // Controlled state synced from props
  const [isGenerating, setIsGenerating] = React.useState(generating)
  const [text, setText] = React.useState<string | null>(transcription)
  ...
  const handleGenerate = () => {
    ...
    // Delegate generation to parent; no local mock updates
    onGenerate?.()
  }
  const headerBadge = (
  ...
  return (
    <div className={["w-full rounded-[5px] border border-[#0033331a] bg-white", className].filter(Boolean).join(" ")} {...rest}>
      ...
      >
        ...
        <svg
          className={"transition-transform duration-200 " + ((expandedProp ?? expanded) ? "rotate-180" : "rotate-0")}
          width="20"
          height="20"
          viewBox="0 0 24 24"
          fill="none"
      ...
      {isExpanded && (
        <div className="px-4 pb-4 pt-0">
          {/* If transcription exists -> show text only box */}
          {text ? (
            <div className="mt-2 rounded-[5px] border border-[#0033331a] bg-white p-4 min-h-[155px] max-h-[155px] overflow-auto">
              <div className="text-[14px] text-[#0f3b3b]">{text}</div>
            </div>
          ) : (
            <div className="mt-2 rounded-[5px] border border-[#0033331a] bg-white p-4 min-h-[155px] max-h-[155px] overflow-auto">
              <div className="text-center">
                <div className="text-[16px] font-semibold text-[#0f3b3b]">AI Call Transcription</div>
                <div className="text-[14px] text-[#0f3b3b]/80 mt-1">Generate a transcript from call recording</div>
